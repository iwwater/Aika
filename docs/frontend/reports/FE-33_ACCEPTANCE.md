...
| FE-33-F | 陪伴会话 | 各项与 FE-31 状态机一致；拖动不算点击；双击不发两轮；quiet 自动发送 0；暂停后普通聊天可用；结束不中断已有轮 | **现场存疑**：点「显示桌宠」看不到桌宠，后续 F 项无法开始 | 待补证（BUG-01） |
...
按 handoff：本份通过后，角色线 FE-27→28→29（需素材与许可证核实），FE-30 最后收口（J～N）。
本份**不构成** FE-30 通过。
