...
> **2026-09-15 口径同步（最新）**：0.5 主线（MVP-01～06）六份 SPEC 已全部执行完毕，收口与遗留清单见 [HANDOFF_MVP_0.5.md](../HANDOFF_MVP_0.5.md)；桌宠按 [PET-01～08](SPEC_DESKTOP_PET.md) 路线，**FE-20 / FE-27 / FE-28 / FE-29 已 SUPERSEDED**（各 SPEC 正文头部有标注），**FE-30 为部分替代**。因此**角色线不再按 FE-27→28→29 派发**；Live2D 归 [PET-08](specs/PET-08.md)（DEFERRED，0.6，需用户先做路线选择）。另：F9 已由 [FE-26](specs/FE-26.md) 交付（09-13，AUTO_PASS）。

> **2026-09-14 最新策略覆盖**：桌宠端只集成现成 Runtime。执行入口改为 [PET-01～08](SPEC_DESKTOP_PET.md)，需求见 [Integration RPD v0.2](RPD_DESKTOP_PET_INTEGRATION.md)。下方“完整桌宠11份SPEC”及 FE-20/27～29 自研路线为历史范围，不能继续据此派发窗口/renderer/口型开发。FE-30/33 只替换其旧桌宠相关验收；FE-31 业务保留、桌宠交互缺口单列。环境/OCR/权限条目与历史报告状态保留。本次仅写规格，不标实现完成。
...
| SPEC | 交付 | 依赖 | 状态 |
| --- | --- | --- | --- |
| [FE-27](specs/FE-27.md) | 素材与运行时核实、manifest、表现契约 | 无环境链路依赖 | **SUPERSEDED（09-14 桌宠新路线）**：不再建自有 manifest/renderer 体系，映射 profile 归 PET-01/02（已交付） |
| [FE-28](specs/FE-28.md) | 真实 Live2D 渲染、表情动作、失败回退 | FE-27；pet集成需FE-20 | **SUPERSEDED**：Live2D 条件接入归 [PET-08](specs/PET-08.md)（DEFERRED，0.6） |
| [FE-29](specs/FE-29.md) | 实际播放能量、口型、打断与降级 | FE-27；真实pet口型需FE-20/28 | **SUPERSEDED**：0.5 不实现桌宠口型，TTS 仍由 Aiki 播放 |
| [FE-30](specs/FE-30.md) | 组合链路、权限、性能与设备验收 | FE-18～22、FE-27～29相关生产实现 | **部分替代**：0.5 桌宠表现按 PET-07 验收，自研窗口/Live2D 组合项不作其门禁；环境、授权、OCR 与陪伴业务项继续保留，**A～I + J～N 不得省略** |

环境线：FE-18→FE-19→FE-22，真实OCR场景需FE-21。FE-21算法前置FE-18，设置集成复用FE-19；算法测试可注入设置端口。窗口线FE-20独立；角色线FE-27→FE-28→FE-29。最后FE-30验组合，一次执行一份SPEC。**（2026-09-15：角色线 FE-27→FE-28→FE-29 已作废，见上表；桌宠改由 PET 线承担。）**
...
| SPEC | 交付 | 依赖与状态 |
| --- | --- | --- |
| [FE-31](specs/FE-31.md) | 开启陪伴统一入口、active/quiet、点击读屏、pet普通输入、暂停/结束 | PARTIAL（2026-09-14 生产逻辑轨）：A~F 全过（22+25+6 定向测试、Rust 31），并**首次把环境链路接进生产装配**；G 真机逐项演示 **NOT RUN**。**09-15 更新**：G 仍 **NOT RUN**（MVP-04-D 的真机证据属「观察→生成→桌宠」链，不构成 G 的会话控制逐项演示）。见[验收报告](reports/FE-31_ACCEPTANCE.md) |
| [FE-32](specs/FE-32.md) | 主显示器前台可见中英文OCR、按需静止读屏、授权文字摘录上下文 | PARTIAL（2026-09-14）：C/D/E 过（投影出口/窗口回退/调度限流）；F 的双语离线加载已验（chi_sim 随包登记，生产路径实测中文/混排识别）；**A（冻结集 CER，缺真实素材）与 B（真机真实 Provider）NOT RUN**。**09-15 更新**：B 取得部分设备证据（真机 OCR→真实 Provider 轮落库，见 MVP-04-D），但「中文/英文/混排各 5 例预标问答」的逐例预标未做；A 仍缺真实素材。见[验收报告](reports/FE-32_ACCEPTANCE.md) |

| [FE-33](specs/FE-33.md) | 环境链路真机装配验收（不含 Live2D） | **进行中（2026-09-14 立项，2026-09-15 加注）**：现场登记 3 个问题（桌宠显示不生效 / 环境感知是否生效不可判断 / 聊天区不能下拉）→ **已定向修复并重建（09-14 17:29）**，独立复验通过（tsc 0 / 53 例 / 835 例 / Rust 31 例全绿）。**FE-33 自身的真机复验仍未跑**（A/B 待补证，C/D/E/G NOT RUN，报告停在 09-14）。09-15 的 MVP-04-D 真机补跑已覆盖其中一段并口径不同：真实画面 OCR → 真实 Provider 轮落库 → 桌宠三类命令全部 accepted（属 *上游 OpenPet* 侧的 device 证据，**不能替代** FE-33 对「新接生产装配在真机是否通」的逐 AC 复验）。见[缺陷记录](reports/FE-33_DEFECTS.md)、[验收报告](reports/FE-33_ACCEPTANCE.md)、[执行手册](reports/FE-33_真机执行手册.md)、[复验清单](reports/FE-33_速查卡.md)。前置 FE-18～22/31/32 已实现且装配已接通，需真机 + 用户在场 + 一份 Provider 凭据。收口 FE-19/20/21/22/31/32 的设备遗留项；**不替代 FE-30** |
