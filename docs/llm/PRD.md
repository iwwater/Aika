...
LLM-04仅单次生成与后台维护，不是ACP外部Agent；复用既有writeback。Live Inspector上下文快照由LLM-11负责，前端FE-23～25消费；不改变检索/预算策略。模型列表历史交付不占用LLM-04后台维护编号。

F9的输入输出分项、后台用途与物理重试记录由[LLM-12](specs/LLM-12_USAGE_LEDGER.md)提供；现有Trace总token不能直接拆账。LLM-12不改变模型请求策略。
