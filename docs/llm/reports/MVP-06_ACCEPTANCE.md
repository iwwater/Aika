...
| F | **FAIL（真实门槛 8/10）+ NOT RUN（Voice）** | 应用内真实 Provider 演示 PASS；LLM-05 真实门槛 8/10（需≥9），「换工作」条目日/英变体偏离来源；Voice 闭环未跑。见 §3 AC-F |
...
### AC-F 真实 Provider 语义演示与 Voice→Agent→Pet 实机闭环 —— FAIL（语义门槛 8/10）+ NOT RUN（Voice）（2026-09-15 补跑）

- **应用内真实语义演示（PASS）**：MVP-04-D 真机补跑中的 proactive 轮即一次真实 DeepSeek 端到端调用——观察进入上下文、回复语义正确引用（详见 MVP-04 报告 AC-D）。
- **LLM-05 AC-D 真实门槛（FAIL，真实发现）**：用宿主存储的密钥（Windows DPAPI 本机解密，用后即删）真实运行 `llm05.real.acd.test.ts`：**8/10 grounded，低于 ≥9 门槛，测试 exit 1**。两处不达标同源——「换工作」知识条目（`k-6b60719d`）的日文（転職の話はどうなった？）与英文（Is she changing jobs）变体回答偏离来源；同条目的中文变体通过。逐题证据已由测试落盘 `docs/llm/reports/evidence/LLM_05_REAL_AC_D_DEEPSEEK_FLASH.json`（10 题 + 5 空检索诚实性观察全部 pass）。
- **Voice→Agent→Pet**：仍 NOT RUN（需真人语音输入）。
- 结论：0.5 DoD 的「真实 Provider 语义」一项**未过线**，冻结发布继续不宣告。
...
**未测 / NOT RUN（需要真人或真实凭证）**：真实 Provider 的端到端语义演示（MVP-04-D、MVP-06-F 前半）；Voice→Agent→Pet 实机闭环（MVP-06-F 后半）；真人声学质量（INT-02，按原规则单独验）；native 同进程崩溃隔离（SPEC 明示不在承诺内）。

**结论**：0.5 的逻辑与模块面已收口；**不宣告冻结发布**——差的是上面四项实机/真实验收。
