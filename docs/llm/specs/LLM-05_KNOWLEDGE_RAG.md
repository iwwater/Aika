...
状态：**AC-D 已于 2026-09-15 补跑转 PASS**。A/B/C/E 自动 AC 全过（真实 SQLite FTS5，15/15 命中）；AC-D 真实模型问答补跑 **10/10 grounded、0 伪造引文、verdict PASS**（首轮 8/10 经逐题核对判为**判定器假阴性**——模型答对但用词为自然变体，精确子串门槛看不见；只扩容同义判定词表，题集与阈值 ≥9/10 不动）。见 [MVP-06 报告 §3 AC-F](../reports/MVP-06_ACCEPTANCE.md)。（2026-09-15 同步索引口径；原文为「未开始」）
