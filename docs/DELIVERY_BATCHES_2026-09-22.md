...
| 批次 | 仓库 | commit | 内容 |
| --- | --- | --- | --- |
| 1 | Aika | `bba4520` | docs: 语音模块文档收口（68 文件：SPEC/报告/索引/台账/.gitignore/AGENTS） |
| 2 | Aika | `73df470` | feat(ser): SER-03/04/05 实现（35 文件） |
| 3 | Aika | `a8dfa9d` | feat(demo): 演示页/打包/视频（33 文件） |
| 4 | Aika | **未提交** | 主工程本地任务改动（8 文件）——交由该功能负责人，不并入语音批次 |
| 5 | GPT-SoVITS | `7fc68b8` | feat(voice): sidecar/网关/探针工具（41 文件） |
| 6 | GPT-SoVITS | `1070c7d` | patch: 上游本地适配补丁 + 日语用户词典（10 文件） |

- 全部为**本地提交，未 push**（push 需另行确认分支与远端）。
- 排除项执行情况：`research/ser/JTES_申请邮件草稿.md`（私人文件）已从暂存区撤出、保持未跟踪；
  GPT-SoVITS 的 951MB `.pth`、`tmp_s1/s2` 保持未跟踪；两仓库 `git diff --check` 通过。
- GPT-SoVITS 仓库为本轮首次提交，已配置与 Aika 相同的本地 git 身份（仅仓库级）。
