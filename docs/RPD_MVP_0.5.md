# Aika 0.5 MVP 收口 — RPD v1.1

> 2026-09-15 文档修订：区分模块交付、产品验收与发行就绪；当前不宣告冻结。此次修订不重开实现任务，历史报告结果按原证据保留。
...
当前回复内嵌 `memoryCandidates` 无生产消费点，已有候选确认路径来自后台抽取；不得据此宣称两路已接通。MEM-DEC-01 保留“接入确认流 / 删除回复候选”的待决策项：前者须覆盖去重、来源、轮次与 epoch 撤权，后者须覆盖 prompt/schema/parser 与旧数据兼容。本次不作实现选择。OCR 不自动长期写入；现有 OCR 当前轮上下文证据不等于长期记忆召回证据。

知识库批量导入见 [KB-01 草案](llm/specs/KB-01.md)，可独立立项，不扩展研究型 RAG，也不把新增草案自动设为 0.5 冻结门禁。

## 验收矩阵与收口

0.5 的未完成清单以 [SPEC 索引当前收口矩阵](integration/SPEC_MVP_0.5.md#当前收口矩阵2026-09-15修订)为准。Voice→Agent→Pet 是其中一项，不能称为唯一欠账；主窗五入口、FE-33 适用项、PET-07 未测项、宿主偶发退出原因与发布门禁分别保留。单次 demo 不替代 INT-02/STT-03 原真人数量；一次稳定运行不证明偶发退出问题已排除。
