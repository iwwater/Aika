...
## 模块化开发规则（2026-09-10 生效）

本节是当前工程执行规则，优先于历史阶段中的串行依赖和全量测试要求。任务先拆成模块 PRD，再拆成模块内多个可独立执行的 SPEC；需求文件统一称 PRD，不另建含义重复的 RPD 文档。

1. **按模块推进。** LLM、TTS、STT、前端分别维护自己的 PRD、SPEC 和验收报告。模块通过显式、可替换接口协作，不能为了开发一个模块强制启动其他真实模块，也不要每次重构整个工程。
2. **文档只有一个根。** 全部项目文档位于仓库根 `docs/`；各模块用 `docs/<module>/PRD.md`、`SPEC.md`（索引）、`specs/`（具体任务）、`reports/`（证据）。`docs/modules/` 只保留共享契约/测试规则，不再复制模块 SPEC。根 AGENTS.md 是 Agent 自动读取的执行规范例外。
3. **一个 SPEC 是一个有界任务。** 写清输入输出、负责的文件/接口、必要前置、实现范围、明确不做、测试名单和 AC；默认只实施当前 SPEC。允许接口 stub 先行，不以其它模块未完成阻塞本模块。
4. **小阶段只测自己。** 针对实际改动运行本模块单元/组件/模块集成测试，其他模块和外部服务使用 fake/mock。被测模块本身必须走生产代码；文件名带 integration 不等于必须启动全产品。
5. **全流程调试按需触发。** 仅在确实需要验证跨模块协作，或已定义的大任务/里程碑完成后，安排端到端调试与全量回归。共享接口变化先测受影响契约；能用窄范围契约测试证明兼容时，不扩大到全系统。完整发布另做构建和安装验证。
6. **不默认重复跑全仓。** 小 SPEC 不默认执行整仓 npm test、全局 tsc/build、Tauri 打包或真实语音链路；不把更换文档/添加报告作为重新测试无关模块的理由。仅修复失败涉及的范围。
7. **验收靠证据。** 每条 AC 标 PASS/FAIL/BLOCKED/NOT RUN，已明确后置项可标 DEFERRED。fixture 证明逻辑编排，真实模型/声学/体验结论必须有对应证据；不把模块通过等同全流程通过。
8. **轻量交付。** 执行者提供改动、定向测试、逐 AC 证据和待联调项；审阅者优先读这些证据，发现问题后定向补测，不重复全量测试。一次任务只维护一份 SPEC 和一份验收报告，不叠加重复计划表。

完整端到端链路为 STT→LLM→TTS→前端，仅在集成阶段运行。LLM 的文本输入/回复可用固定文本与 headless harness 独立验收；TTS 用固定文本和输出引擎 fixture，STT 用音频/识别 fixture，前端用假 Runtime。实时语音/环境感知/Live2D 的既有后置安排保持，不因拆文档自动恢复。
