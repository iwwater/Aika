...
更新日期：2026-09-26（合批后对齐 0.81/0.82 状态；其他版本仍以各自最新 SPEC/报告为准）。
...
| 0.81 | **N081-00～06 有实现及定向证据，07 工具已实现；版本未验收** | [SPEC](next/0.81/SPEC.md)、[报告](next/0.81/reports/)、[两阶段验收计划](next/0.81/ACCEPTANCE_PLAN.md) | G1～G3 有实机记录；G4/G5、性能阈值、自然试运行与人工确认未完成，不得标 ACCEPTED |
| 0.82 | **BLOCKED：CR-02/03 与依赖的实机验收 DEFERRED** | [SPEC](next/0.82/SPEC.md)、[审阅与修复记录](next/0.82/reports/CODE_REVIEW_20260926.md) | CR-01、04～10 定向回归通过；正式抓屏/OCR、正文来源与自动陪伴闭环按用户决定后置，43/43 自动测试不构成版本验收 |
...
UI 专项的历史 PASS 报告不覆盖正式 Playground 接线和角色预设生产作用域缺口，当前为 **REPAIR_REQUIRED**。按 [MVP 修复方案](next/ui-rework/MVP_REPAIR_PLAN_20260925.md) 收敛页面、复现并修复卡顿；原 [PRD](next/ui-rework/RPD.md)、[SPEC](next/ui-rework/SPEC.md) 与报告保留为历史依据。0.81/0.82 的当前状态以上表和逐项报告为准。
