...
**执行状态（2026-09-17 更新）**：用户已拍板三项决策（**代码位置：Aika 仓库 `pet-shell` 分支**、自用不分发、MVP-12 纳入且仅点击交互）并授权执行全部未完成 SPEC。

> **仓库形态变更（2026-09-17，用户决定「不新开仓库」）**：shell 源码**不再**只存在于本机独立仓库 `f:/AIVoice/pet-shell`，已推送为 **Aika 仓库（`https://github.com/iwwater/Aika.git`）的 `pet-shell` 分支**，含 `aiki/0.6` 的 24 个提交完整历史。
>
> - **本地工作副本**仍在 `f:/AIVoice/pet-shell`，两个远端：`aika`（→ Aika 仓库 `pet-shell` 分支，日常推送改造）、`upstream`（→ `X-T-E-R/OpenPet`，保留以便继续同步上游）。
> - **两个分支历史独立**：`master` 是 Aika，`pet-shell` 是 shell；切换用 `git checkout pet-shell` / `git checkout master`，**不是子目录**。
> - 历史报告里的「双仓」一词指当时两仓库分立的形态，属**当时事实，不回溯改写**；统一说明以本节为准。
> - 影响：MVP-08 的品牌资产改名范围、MVP-09 的 fixtures 同步范围不变，只是 shell 侧的落点从本机仓库变为本仓库的 `pet-shell` 分支。
...
- MVP-15：**B 六条不变量实测通过，真实应用已验到出声调用**（[报告](reports/MVP-15_ACCEPTANCE.md)、[明细](specs/MVP-15_CLICK_CONSUMER.md)）——2026-09-16 用户选 **B（受限轻量回应）**：点击 → 固定短语池的一句（不调 provider）× `canSend` 终审 × ≥30s 冷却 × 不排队 × 不打断 × 不冒错。模块 11 项 + 接线 8 项 + 全量 156 文件 / 1701 项通过；真机端到端：点击被受理 → `speakAside` 被调用（捕获到「嗯？」）→ 30s 内第二次点击**受理但不出声**。新增 `CompanionPresenter.canSpeakAside()` 与 `VoicePresenter.speakAside()/isSpeaking()`。**2026-09-16 按用户反馈调优**：冷却 30s→**5s**；回应同时**冒桌宠气泡**（`/api/say`）；说话中的新点击只计数、**不吞掉正在说的话**（真排队需改 `SpeechQueue` 回合语义，未做）。**两项待办**：① 音频人耳确认（本轮拦下合成调用取证，未真的播放）；~~② 默认不出声~~ **→ 已按 (b)+(ii) 解决**：独立开关 `pet.clickReaction` **默认开**（UI「主动 / 被动响应」里的「点击桌宠时应一声」），且 **`canSpeakAside` 不受「主动消息」总开关影响**（那个开关管她主动开口；勿扰时段/每日上限/最小间隔仍生效）。C/D 未采用。
...
| 决策 | 当前状态 | 影响 |
| --- | --- | --- |
| 正式产品名、代码位置 | **代码位置已定：Aika 仓库 `pet-shell` 分支**（2026-09-17；此前为本机 `f:/AIVoice/pet-shell`，2026-09-16 口径）。**独立仓库名问题随之消失**——不再另设仓库；正式产品名仍待定，文档暂用 PetShell / pet-shell | MVP-08 品牌资产（改名要同步 profile 与契约） |
...
- shell 源码在 **Aika 仓库的 `pet-shell` 分支**（2026-09-17 起；此前为本机独立仓库）；Aika main 侧仅允许 SPEC 明列的 adapter/profile/进程管理最小增量。不得恢复旧 PetApp 或扩大到 Agent、Memory、OCR 重构。
