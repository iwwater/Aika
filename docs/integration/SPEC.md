...
| AC | 后续操作与通过条件 |
| --- | --- |
| INT-03-NSIS | 先核对失败日志、工具缓存及构建阶段，再进行一次定向 NSIS 重试；记录命令、退出码、工具版本、产物哈希。`timeout: global` 是已观察错误，工具链下载是否根因须验证；失败后记录阻塞，不无限重试。旧 NSIS 包不能充当当前基线成果，不为此重跑无变化 MSI |
| INT-03-INSTALL | 在明确授权的测试环境按实际安装作用域验安装→启动→退出→卸载；不默认要求用户日用机器或系统级安装。先明确用户数据保留规则，检查文件/快捷方式/进程残留；无授权仍 NOT RUN |
| INT-03-STABILITY | 核对 Handoff 偶发退出是否人工关闭、工具终止或应用异常，留存时间线与可得退出码/日志；按原条件持续复验，无法复现时如实记录观察窗口。证据不足不得判已修复 |
| INT-03-RELEASE | 逐列确认本次交付适用 INT-01/02、设备与安装器证据；区分历史报告、当前构建与实际发布产物。不以 MSI 可构建、一次语音 demo 或新版本 SPEC 完成代替全部适用门禁 |

本段只补规格，不重写历史验收报告、不启动构建或安装。0.5 当前欠账见 [MVP 0.5 索引](SPEC_MVP_0.5.md)，0.6 新工作见 [MVP 0.6 草案](SPEC_MVP_0.6.md)。
