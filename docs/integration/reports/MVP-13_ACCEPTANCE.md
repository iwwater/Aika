...
本轮真机验收抓到四个缺陷：**DEF-1** 包内容与许可前提不符（已由 MVP-14 修复）；**DEF-2** Live2D 模式下整窗鼠标穿透（已修复并人工复验）；**DEF-3** 导入宠物在目录名 ≠ 清单 id 时贴图 URL 取不到（已修复并真机复验）；**DEF-4** 宿主托盘「完全退出」不回收 owned 桌宠（2026-09-16 用户目视发现，已修复，待用户复测托盘路径）。
...
| **DEF-4 托盘「完全退出」遗留 owned 桌宠（孤儿）** | **用户目视发现（2026-09-16），已修复，待托盘路径复测**。根因：托盘退出是 `app.exit(0)` 立即退出，PET-05 的正常清理点（JS `dispose()`）不会执行，`stopOwnedOnExit` 的承诺落空。修复：承诺在 **spawn 时定死并下沉到 Rust**（`desktop_pet_process_spawn` 新增 `stopOnHostExit`，管理器按配置传入），托盘退出先 `stop_all_on_host_exit()`（有界等待、只带带承诺的）再 `app.exit(0)`。测试：Rust 新增「只带走带承诺的」用例（13 项通过）；TS 断言 spawn 选项含 `stopOnHostExit`（desktopPet 103 项通过）。真实托盘路径由用户复测 |
