...
### 5.2 补充证据：注入在真实两进程链路上生效（A/B）

为把「Aiki 真的把凭据交给了它派生的那个子进程」与「Aiki 自己的接收端是否正常」分开，
做了一次 A/B：**仍由 Aiki 派生**（走真实 `desktop_pet_process_spawn` 命令与真实环境变量注入），
但把回调地址指向一个受控的假接收端。

```text
desktop_pet_process_spawn(path, clickUrl:"http://127.0.0.1:8766/api/pet/click",
                               clickToken:"spawn-ab-token-0123456789") → {"pid":1712}
OS 层点击 6 次（窗口中心 3 次 + 中心下方 70px 3 次）
→ 假接收端日志 +2 条，Authorization: Bearer spawn-ab-token-0123456789（正是经 Aiki 注入的那枚）
  body: {"schemaVersion":1,"type":"click","eventId":"1789558724650-1","atMs":…,"payload":{"button":"left"}}
```

结论：**注入路径 PASS**（Aiki → 子进程环境变量 → 子进程按注入值投递），与 Aiki 自身接收端
的正确性互为独立证据。到达率偏低（6 → 2）的原因见 §7 末条，属取证装置局限。
...
**未闭合 2（部分完成）**：真实进程中「Aiki 不在线」的断连分支——已在真实进程上确认**行为面**：Aiki 被强杀后对宠物连点 5 次，宠物四端点仍正常（`GET /api/status` 200 / 34 ms、`POST /api/say` 19 ms）、进程存活、窗口可见，**没有被重试拖累**。但「这几次点击是否真的产生了投递尝试」无法确认：宠物的投递失败计数不在 `/api/status` 暴露。计数面仍由 fixture 覆盖。

**未闭合 3 → DEFERRED（改为后续单独测）**：双击折叠的 400 ms 边界在真实两进程链路上未隔离。原因两条：① 精确测量需要宠物的 CDP 输入，而 **Aiki 持有 WebView2 调试端口时宠物无法再开一个**（WebView2 按用户数据目录复用浏览器进程，同一进程只能有一个调试端口，实测 `--remote-debugging-port=9333` 无法绑定）；② 退回 OS 层合成点击会**移动用户光标**，而当时用户正在用机器做别的事——已按用户要求停止该路线。后续测法（已探明可行）：停掉 Aiki → 以 `--remote-debugging-port=9333` 单独派生宠物并把回调指向假接收端 → CDP 精确注入点击。

**顺带记录（不作为产品结论）**：OS 层合成点击在宠物窗口上的到达率不稳定（同一位置连点 6 次 → 接收端 2 条；单次会话内 3 次 → 1 条），因此**不能用它测量折叠边界或丢失率**。这是取证装置的局限，不代表真人点击会丢——§5.1 的真人操作证据（1 次点击完整走通全链路）与单进程链路的 CDP 精确证据（4 → 3，双击折叠）都成立。
