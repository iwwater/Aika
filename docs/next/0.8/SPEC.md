...
| **08-01 事件与 Timeline** | 在现有 `HostEventChannel` 上对接正式生产事件（Canon/Companion/Work 分域）；实现幂等投影、重启恢复与旧来源失效。 | 重复、乱序、重启、撤销、角色隔离与失败重试测试通过；真实对话事件可追溯。 | **PENDING** |
| **08-02 会话与桌宠核心交互** | 气泡、展开聊天和控制台会话查看共享单一 `TurnScope` 权威；保留语音输入、Work 卡、取消与原话定位。 | 收起/展开不新建轮次、不重复触发 TTS；网络抖动、历史分页与鼠标穿透正确生效。 | **PENDING** |
| **08-03 授权感知** | 窗口/区域选择、单次/会话级 `CaptureGrant`、预览与撤销；OCR/VLM 适配器；时效性 `Observation` 仅作为动态后缀注入 Context。 | 拒绝/撤销/锁屏/目标消失后零新采集；本地/云端切换重新授权；固定图与真实 Windows 证据。 | **PENDING** |
| **08-04 主动陪伴** | 统一免打扰、配额、冷却、去重和气泡展示仲裁；先支持临时事件，长期画像路线受门槛控制；邀请与动作分离。 | 假时钟与真实场景覆盖接受、忽略、停止、撤销、角色切换；绝不自动开麦或执行命令。 | **PENDING** |
| **08-05 Work/ACP/MCP** | 保留现有 Work 确认与回执权威，分别实现一个已冻结版本的 ACP 适配器和 MCP 工具路径。 | 重复确认只派发一次；未知结果不盲重发；各有一条真实兼容服务证据。 | **PENDING** |
