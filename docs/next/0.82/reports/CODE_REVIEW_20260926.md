...
## 修复进展（2026-09-26，隔离 worktree）

| 项 | 当前状态 | 证据与边界 |
| --- | --- | --- |
| CR-01 | PASS（定向自动化） | 旧授权停采、迟到回调拒绝及释放失败回归通过；真实锁屏/重启组合根回归 NOT RUN |
| CR-02 | BLOCKED | 移除合成帧与 PNG 元数据伪 OCR，缺适配器时报不可用；真实抓屏和本地像素 OCR 尚未接线及实机验证 |
| CR-03 | BLOCKED | 正式正文来源、每日 timer、周期观察 context/tick 与陪伴输出未闭环 |
| CR-04/05/06/07/08/09/10 | PASS（定向自动化） | 各项定向回归通过；详情见 N082-02/04/06 |

当前 `npm run test:next082`：43/43 PASS、退出码 0（含编译）；`node --check desktop/electron/main.mjs`：PASS、退出码 0。上述自动化结果不能替代 CR-02/03 的正式入口和实机验收。

## 用户后置决定与本次 Git 合批

2026-09-26，用户明确将 CR-02 的真实抓屏/像素 OCR、CR-03 的正式来源与自动调度/陪伴闭环，以及依赖它们的实机验收后置。状态改记 **DEFERRED**；当前 0.82 版本仍 **BLOCKED**，不得使用 `READY_FOR_HUMAN` 或 `ACCEPTED`。已完成的 CR-01、04～10 定向修复可以随本地与云端代码历史合批；Git 合批不代表 0.82 功能验收完成。
