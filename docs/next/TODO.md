...
| ID | 需求 | 设计要点与建议归属 |
| --- | --- | --- |
| R-TODO-18 | **【后续优化·待办】压制事实沉淀速度（记忆提炼频率控制）**：用户明确指出：当前 MVP 版本的长期事实沉淀频率过快（每轮对话后立即触发异步提炼）。正常生产环境应该严格压制事实沉淀的速度（例如：每次应用开启时才集中分析上次未沉淀的对话、或者每隔固定时间段/累积一定轮次后批处理触发，甚至在后台空闲闲时执行维护），避免高频调用模型产生不必要消耗。当前测试阶段保留每轮提炼便于单步验证，后续记得优化沉淀频率策略。 | **【后续优化·待办】**（2026-09-22 用户明确指示：测试阶段保持当前链路用于快速验证，后续必须改）。<br>① **现状**：MVP 链路实现了对话后异步后台提炼并写入 SQLite 证据链，链路已通；<br>② **优化方向**：<br>　• **按会话/启动批处理**：桌宠启动或会话关闭时扫描未提炼的 transcript 进行批量记忆提取；<br>　• **滑动轮次节流**：累积 5~10 轮有效对话后触发一次增量摘要与事实抽取；<br>　• **定时闲时维护**：对接现有 `maintenance_tasks` 机制，在用户空闲时静默提炼并归档。归属建议：**0.7 / 0.75 记忆管理优化待办** |
