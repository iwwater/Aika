...
| ID | 需求 | 设计要点与建议归属 |
| --- | --- | --- |
| R-TODO-18 | **【N075-01 正式化 + 后续优化】记忆提炼生命周期与频率策略**：用户明确指出：当前 MVP 版本的长期事实沉淀频率过快（每轮对话后立即触发异步提炼），正常生产环境应严格压制事实沉淀速度；同时 N075-01 前置修复要求所有生产 Distill 必须经过正式生命周期，**正确的生命周期 > 减少几次 LLM Call**。 | **【N075-01 落地 + 后续优化】**（2026-09-22 修订，拆分生命周期语义，替代原"每 N 轮一次"的单句表述）。<br>① **生命周期语义（N075-01 硬要求，立即生效）**：<br>　• **Raw Transcript** → 永远立即保存（immediate durable write），任何策略不得延迟；<br>　• **correction / forget / uncertain** → 立即进入 strict memory handling，**Pending privacy guard 不允许等待 batching**；<br>　• **普通事实提炼（ordinary additive memory）** → 可后台 queue / debounce / batch / idle，当前允许 per-turn background plan；Foreground Reply 不等待普通 Distillation；<br>　• **Summary** → 保留现有 minMessages threshold scheduler；<br>　• **所有生产 Distill 必须经 `MemoryTurnPlan → validation → SqliteLifecycleState.commitTurn()`**，禁止 Production direct SQL 写 `memory_records` / `memory_search`（FE75-08 迁移要求）；<br>② **频率策略（后续优化，不在 N075-01 强行完成复杂 batching）**：<br>　• 按会话/启动批处理：桌宠启动或会话关闭时扫描未提炼 transcript 批量提取；<br>　• 滑动轮次节流：累积 5~10 轮有效对话触发一次增量摘要与事实抽取；<br>　• 定时闲时维护：对接现有 `maintenance_tasks` 机制空闲提炼归档；<br>③ **权威**：foreground context 与 background memory 处理以现有 `BackgroundMemoryPort` / `RoleMemoryLifecycleQueue` 为准；归属：**N075-01 正式化 + 0.7/0.75 记忆管理优化待办** |
