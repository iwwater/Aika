...
详细现象、目标行为和验收标准见 [FIELD_TEST_NOTES.md](../stt/further/FIELD_TEST_NOTES.md)。
...
## 接下来的顺序（2026-09-03 重排）

上一版那个九步清单里的 1–6 和 8 都做完了，已经删掉，避免和现状对不上。
完整的阶段划分与排序理由见 [DEVELOPMENT_PLAN.md](DEVELOPMENT_PLAN.md) 的
**「接下来的开发计划」**，动手清单见 [NEXT_STEPS.md](NEXT_STEPS.md)。

摘要：

- **阶段 0 ← 现在在这里**：把已经写完的东西验掉。起 whisper-server、手机上打开一次、
  `output/` 冷备份，都是十分钟以内的事，但**没做完之前不要开新的里程碑**——
  现在有四个「代码写完、真机没验」的东西堆着（M1、M2、M3、M7）。
- **阶段 1**：表情包素材放进来。机制已完成，性价比最高的一件事。
- **阶段 2**：M4 拆成两半。**窗口那一半不依赖模型来源，先做**——它能立刻回答
  「挂在桌角两小时会不会烦」，而这个答案会反过来影响要不要花钱买模型。
- **阶段 3**：M5 声线。数据四步先做，训练等 M1/M2 验收通过。
- **阶段 4**：M7 第二版（手机上的语音、表情包、通知）、M6 自制角色（暂缓）。

明确不做：用户情绪检测与声学情绪、公网穿透、多角色与云同步。

自制角色的 PSD 拆分与 Cubism 绑定仍推迟到产品验证之后（M6），素材全部保留。
