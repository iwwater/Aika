...
| 项 | 09-15 状态 | 说明 / 去向 |
| --- | --- | --- |
| A1 命令下行真实进程复测 | **NOT RUN（2026-09-17 用户决定本轮不再尝试）** | 服务端实测健康（8765 LISTENING、`GET /` 200 返回完整手机页、无凭据 `POST /api/v1/commands` 正确返回 `{"error":"token 不对"}`），但本机浏览器必须走带 token 的地址，用户不接受该入口。**这是 0.5 唯一「本地证据只到 HTTP 202」的断点，仍然悬着**，根因与修复方向见 [HANDOFF §二.7 TOFIX-01](HANDOFF_MVP_0.5.md) |
...
| A4 27 份 AUTO_PASS 报告审阅 | **机械扫描已完成，人工判定仍待** | 2026-09-16 全仓 99 份 `*_ACCEPTANCE.md` 扫「证据命令 + 退出码 + NOT RUN 声明」三件套：**0 份真缺陷**（唯二命中的两处是归档件与 `llm/reports/LLM-04_ACCEPTANCE.md` 的历史重定向桩）；首轮启发式报出的 5 份「疑似缺退出码」经逐份复核为**假阳性**（写的是 `exit 0` 而非「退出码 0」：`core/MVP-01`、`frontend/MVP-02`、`frontend/PET-01`、`frontend/PET-02`、`llm/MVP-06` 均有命令且均有 exit 0）。**人工判定部分仍未做**（这才是 A4 的本体）；09-15 新增的 INT-03 报告同样待审阅 |
| B1 界面与交互目视 | **部分** | INT-03 的重开渲染回归已取主窗像素（角色区 / 对话区 / 侧栏渲染正常、历史从库加载成功）；FE-23/24/25/26、TTS-04、FE-02 各专项页签**未逐一目视** |
| B2 INT-01 桌面列六项 | **部分** | Tauri+plugin-sql 列已取部分设备证据（真机读到历史消息；`trace_events` 由真实应用经 plugin-sql 建表并写入）；**「错误 UI」未目视**；「生产 DEV 默认开关」已于 2026-09-16 取构建级证据（`npm run build` 退出码 0，产物 `import.meta.env` 引用 0、Trace 默认 `false`）→ 整列仍不 PASS |
...
> **2026-09-17**：本轮把下列人工项打包成一次会话执行脚本（含每站步骤、预期、证据要求）→ [验收会话手册 · 2026-09-17](ACCEPTANCE_RUNBOOK_2026-09-17.md)。本文件仍是人工项的权威清单；手册是一次性脚本，会话结束后结论回填到本文件与 [HANDOFF_MVP_0.5.md](HANDOFF_MVP_0.5.md)。
...
**2026-09-16 机械扫描结果**（全仓 99 份报告的自动化检查，代替不了人工判定，但先把「有没有证据」这一层筛掉）：

- 缺「证据命令」一节：**0 份**。唯二命中是归档件与 `llm/reports/LLM-04_ACCEPTANCE.md`——后者已是历史重定向桩（指向 `LEGACY_MODEL_LIST_ACCEPTANCE.md`），不是缺陷。
- 缺「退出码」：**0 份**。首轮启发式报出的 5 份（`core/MVP-01`、`frontend/MVP-02`、`frontend/PET-01`、`frontend/PET-02`、`llm/MVP-06`）逐份复核为假阳性：它们写的是 `exit 0`，且命令与结果都在。
- 因此**剩余的只有人工判定本身**：报告结论是否与证据相符、NOT RUN 的边界是否诚实——这一层需要你（或至少是独立复核）来做，机械扫描给不出结论。
...
**不受影响（原判，2026-09-17 已被推翻）**：曾认为 A1 改走本机浏览器（`127.0.0.1:8765`）即可完成、无需手机。**实测该路径同样走不通**：Rust 鉴权只认 `Authorization: Bearer` 或 `?token=`，没有来源例外，本机裸地址被当作远端设备，页面提示「少了 token」。见 [HANDOFF §二.7 TOFIX-01](HANDOFF_MVP_0.5.md)。
