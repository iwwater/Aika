# 验收计划：留给你的验收清单（2026-09-13 23:20 更新）
...
3. 手机与宿主同一局域网，手机浏览器打开 `http://<宿主IP>:8765/`，输入 token。
4. 手机页发一条可判定的消息，例如：「请只回复四个字：收到命令」。
5. 同时观察桌面端界面与手机页。
...
### B4 · 真实手机浏览器连宿主（20 分钟，需你的手机）

配对码配对、断网重连、能力降级目视；租约在线/离线正确、重连不串数据（GW-04 设备端到端）。
注：Tailscale/Origin 白名单属远程联调里程碑，可后置。
