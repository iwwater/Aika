...
| [CORE-05](specs/CORE-05_PLUGIN_CAPABILITY.md) | 插件契约、既有能力插件化、扩展点验证 | CORE-04 | A–F 自测 PASS；voice/stickers 能力插件 + 示例插件；输入用例包两实现全绿；**G 输出侧 BLOCKED**（`stash@{0}` 不存在）；待审阅 |
