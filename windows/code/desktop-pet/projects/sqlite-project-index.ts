...
function path(value:unknown,max:number,absolute:boolean):string {
 ...
 const drive=absolute&&process.platform==='win32'&&/^[a-z]:\//i.test(normalized);
 if(isAbsolute(result)!==absolute||(!drive&&/^[a-z][a-z0-9+.-]*:/i.test(result))||(!absolute&&result.includes(':'))||result.includes('://')||/[\r\n]/.test(result)||normalized.split('/').includes('..'))return invalid();
