...
export async function readSetupKey(file:string,projectRoot:string):Promise<string>{
 ...
 if(!isAbsolute(file)||!isOutside(root,actual)||before.isSymbolicLink()||!before.isFile()||!isPrivateFileSync(file,before)||before.size>4096)throw Error('credential_unavailable');
 const h=await open(actual,constants.O_RDONLY|constants.O_NOFOLLOW);
 try{const after=await h.stat();if(!isPrivateFileSync(actual,after)||after.size>4096||!after.isFile())throw Error('credential_unavailable');
