...
export async function privateDirectory(path: string): Promise<string> {
  ...
  for (const part of absolute.slice(current.length).split(sep).filter(Boolean)) {
    ...
    try { await mkdir(current, { mode: 0o700 }); restrictPrivatePathSync(current); } catch (error) { if ((error as NodeJS.ErrnoException).code !== 'EEXIST') throw error; }
...
export async function privateRead(file: string, limit: number): Promise<Buffer> {
  ...
  privateMode(stat,0o600); if(!isPrivateFileSync(file,stat))throw new VoiceReferenceError('unsafe_storage');
  const handle = await open(file, constants.O_RDONLY | constants.O_NOFOLLOW);
  try {
    const actual = await handle.stat();
    if (!actual.isFile() || actual.size > limit) throw new VoiceReferenceError('unsafe_storage');
    privateMode(actual,0o600); if(!isPrivateFileSync(file,actual))throw new VoiceReferenceError('unsafe_storage');
    const bytes = await handle.readFile();
    try {
      const after = await handle.stat(), pathAfter = await lstat(file);
      privateMode(after,0o600); privateMode(pathAfter,0o600); sameFile(actual,after); if(!isPrivateFileSync(file,after))throw new VoiceReferenceError('unsafe_storage');
...
export async function privateJson(file: string, value: unknown): Promise<void> {
  ...
  try {
    ...
    try { restrictPrivatePathSync(temporary); await handle.writeFile(JSON.stringify(value) + '\n'); await handle.sync(); } finally { await handle.close(); }
...
export class VoiceReferenceStore {
  ...
  async save(input: { bytes: Uint8Array; filename: string }, signal: AbortSignal): Promise<VoiceReference> {
    ...
    try {
      ...
      try { restrictPrivatePathSync(file); await handle.writeFile(bytes); await handle.sync(); } finally { await handle.close(); }
      requireLive(signal);
