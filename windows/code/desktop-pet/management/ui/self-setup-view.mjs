...
const providers=[{value:'deepseek',label:'DeepSeek · 文本模型'},{value:'dashscope',label:'阿里云百炼 · 转写、多模态与 MiniMax'},{value:'gemini',label:'Google Gemini · 文本模型'}];
...
export function createSelfSetupView(client,render,host){
 ...
 function accept(value){
  if(value?.apiVersion!==1||!text(value.instanceId)||!['first-run','runtime'].includes(value.mode)||!whole(value.credentialRevision)||!Array.isArray(value.credentials)||!value.credentials.every(c=>text(c.id)&&providers.some(p=>p.value===c.provider||p.value===c.provider?.split('-')[0])||text(c.provider)&&['configured','missing','unavailable'].includes(c.status))||!validSettings(value.settings)||!Array.isArray(value.adapters)||!value.adapters.every(a=>text(a.id)&&providers.some(p=>p.value===a.provider||p.value===a.provider?.split('-')[0])||text(a.provider)&&Array.isArray(a.models)&&Array.isArray(a.slots)&&Array.isArray(a.endpoints))||!Array.isArray(value.references)||!value.references.every(validReference)||!Array.isArray(value.operations)||!value.operations.every(validOperation)||!value.initialization||!Array.isArray(value.initialization.blockers)||!value.links)throw Error('Invalid setup snapshot');
