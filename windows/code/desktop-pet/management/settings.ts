...
export function validateManagedSettings(value: unknown, base: TrialConfiguration, voices?: RegisteredVoiceStore, historical = false, credentials = credentialRegistry(base), draftOnly = false): ManagedSettings {
  ...
  for (const slot of PROVIDER_SLOTS) {
    ...
    // Endpoint may be any HTTPS origin or an explicit loopback HTTP; the key never reaches a
    // redirected other host. Catalog endpoints are HTTPS and remain accepted.
    if (!adapter || adapter.provider !== p.provider || !isAllowedEndpoint(p.endpoint) || !text(p.model) || !text(p.credentialRef)) invalid();
    ...
    // For a reviewed choice we still cap usage at the reviewed bound; custom models are user-bounded.
