...
export class ManagementSettingsStore {
  ...
  static async open(file: string, base: TrialConfiguration, voices?: RegisteredVoiceStore): Promise<ManagementSettingsStore> {
    ...
    try {
      ...
      state.current.settings = validateManagedSettings(state.current.settings, base, voices);
      ...
      for (const item of state.history) item.settings = validateManagedSettings(item.settings, base, voices, true);
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code !== 'ENOENT') throw error;
      state = { version: 1, current: { revision: 0, savedAt: new Date().toISOString(), settings: defaultManagedSettings(base) }, history: [] };
    }
    return new ManagementSettingsStore(file, base, state, structuredClone(state.current.settings), state.current.revision, voices);
  }
  ...
  save(expectedRevision: number, settings: unknown): Promise<SettingsSnapshot> { return this.commit(expectedRevision, () => validateManagedSettings(settings, this.base, this.registeredVoices)); }
  rollback(expectedRevision: number, targetRevision: number): Promise<SettingsSnapshot> {
    return this.commit(expectedRevision, () => { const target = this.state.history.find(item => item.revision === targetRevision); if (!target) throw new ManagementError('not_found', '没有这个可回滚配置版本。'); return validateManagedSettings(target.settings, this.base, this.registeredVoices); });
  }
