...
export class WakeManager implements WakeManagement {
  ...
  static async open(options: Options): Promise<WakeManager> {
    ...
    try {
      const stat=await lstat(options.file);if(!isPrivateFileSync(options.file,stat))throw Error('Unsafe wake settings');
  ...
  async save(instanceId:string, expectedRevision:number, value:unknown):Promise<WakeSnapshot>{
    ...
    try {
      ...
      restrictPrivatePathSync(tmp);await rename(tmp,this.options.file);this.settings=settings;this.revision++;return this.snapshot();
