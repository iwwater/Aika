...
test('missing or changed hold trigger never lowers the privacy boundary to zero',async t=>{
 ...
 const db=new Database(f.filename);f.track(db);db.prepare("UPDATE memory_pending_mutations SET current_message_id='missing' WHERE current_message_id='held:user'").run();
