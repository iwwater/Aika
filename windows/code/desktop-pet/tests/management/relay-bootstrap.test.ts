...
test('actual management bootstrap preserves companion/ledger bytes and separate index/receipts across restart', async t => {
  const f = await fixture(t), directory = join(f.c.projectRoot, 'data/desktop-pet'); await mkdir(directory, { recursive: true });
  ...
  assert.equal((await stat(indexFile)).mode & 0o077, 0); assert.equal((await stat(receiptsFile)).mode & 0o077, 0);
