> **Windows 开发版：** 请先阅读 [README-WINDOWS.md](README-WINDOWS.md)，或双击 `Start-Windows.cmd`。下方保留原 macOS 项目说明。
...
<div align="center">
  ...
  <p><a href="docs/SETUP.md">开始使用</a> · <a href="docs/MEMORY.md">记忆与上下文</a> · <a href="docs/LIVE2D.md">接入自己的 Live2D</a> · <a href="assets/original-design/README.md">DeepSeek 大肥鱼设计稿</a></p>
</div>

---

AAAAGENT 是一个面向 macOS 的陪伴型桌面助手。你可以和它说话、用文字交流，也可以通过微信延续对话。需要完成工作时，它把需求整理成任务卡，交给你确认，再转给 DeepSeek Harness 或已有的 Codex 任务。
...
先阅读 [安装与配置](docs/SETUP.md)。这是需要本地配置的开发者版本，完整桌宠还需要你自行准备合法授权的 Live2D 模型及 SDK。
...
code/desktop-pet/          后端、桌面、网页、模型适配与测试源码
tools/                    发布检查与配置辅助工具
docs/                     安装、记忆、模型接入说明
...
- 本仓库面向 macOS 源码开发；未声明 Windows / Linux 桌面体验已验证。手机实际显示、音色效果及新模型动作需要在自己的设备验证。
