...
- 连续语音模式：自动听写、自动继续聆听，她说话时你直接开口就能打断
- 句中停顿、犹豫、换气不会被当成说完：听到的内容先显示出来，可以「现在就发」或「清掉」
- 语音识别语言跟着你最近说的话自动走，没有手动切换按钮；回复逐句判断音色，她说哪种语言就用哪种音色念
...
- 字幕跟着朗读逐句高亮：她念到哪儿，字幕就亮到哪儿
- 语气标签：每条回复带一个受控语气（七个，对齐 Live2D 表情预设），由她自己标，现在驱动朗读的语速与音高，M4 之后驱动表情
- 表情包机制：她只能从你放进 `public/stickers/` 的固定清单里挑，不生成图；清单为空时整套机制隐身
- 手机也能用：局域网里开一个带 token 鉴权的页面，手机上接着聊。数据仍然只有电脑上那一份，手机只是一块屏幕
...
- 表情包需要你自己把图片放进 `public/stickers/` 并写明各自的使用场景，见那个目录里的 README。没放素材时她不会提这件事。
