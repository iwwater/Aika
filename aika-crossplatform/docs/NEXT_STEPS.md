...
## 语言：不分两个方向（2026-09-02 追加）

- 提示词不再是「默认某语言 + 满足条件才切换」的二元开关。她是一个双语者，
  哪个词先到嘴边用哪个，一句话里混着说也正常——不是两个模式。
- 语音页的「日语 / 中文」按钮已删除。识别语言由 `domain/language.ts`
  按用户最近说的话推导（移植自 Android `LanguageDetector`），默认日语。
- **这只是折中。** Web Speech 一次只能给一个语言码，一句话里中日混说仍然识别不准，
