...
## 语言：不分两个方向（2026-09-02）

- 提示词不再写「默认某语言，满足条件才切换」。她是一个双语者，不是两个模式。
- 语音页的「日语 / 中文」按钮已删除。识别语言跟着用户最近说的话走
  （`domain/language.ts`，移植自 Android `LanguageDetector`），默认日语。
- **Web Speech 一次只能给一个语言码**，一句话里中日混说仍然识别不准；
  真正的自动识别要等 M3 的本地 Whisper。折中方案不假装已经解决。
- 气泡与字幕仍是「原话 + 中文意思」两层——那是显示层，不是她脑子里有两种语言。

## 最新实机反馈

- P0：Web Speech 会在用户句中停顿时过早提交，造成抢话和发言被截断。
- P0（按钮已移除，完整方案待 M3）：不应要求用户手动选择日语或中文。
