...
if(arg==='--activate-existing'){
  ...
  for(const m of Object.values(config.models)){
    ...
    if((!local.startsWith('..')&&!isAbsolute(local))||!info.isFile()||(info.mode&0o077)!==0||info.uid!==process.getuid())throw Error('External credential file permissions changed.');
  }
...
if(process.platform!=='darwin')throw Error('The native desktop target currently requires macOS.');
const code=resolve(root,'code/desktop-pet'),runtimeFiles={};
const hash=b=>createHash('sha256').update(b).digest('hex');
async function pin(base){for(const entry of await readdir(base,{withFileTypes:true})){const path=resolve(base,entry.name);if(entry.isSymbolicLink())throw Error('Runtime symlinks are not accepted');if(entry.isDirectory()){if(!['.cache','.git','node_modules'].includes(entry.name))await pin(path);}else runtimeFiles[relative(root,path)]=hash(await readFile(path));}}
...
for(const model of Object.values(input.models??{})){
  ...
  if(!local.startsWith('..')&&!isAbsolute(local)||!info.isFile()||(info.mode&0o077)!==0||info.uid!==process.getuid())throw Error('Credential files must be outside this package and mode0600.');
  model.credentialFile=path;
}
const sourceRevision=hash(JSON.stringify(runtimeFiles)).slice(0,40);
const config=validateTrialConfiguration({version:1,product:'companion-v1',phaseId:'local-trial-personal',purpose:'user-trial',projectRoot:root,sourceRevision,runtimeFiles,
...
if(!registryRelative.startsWith('..')&&!isAbsolute(registryRelative))throw Error('Keep the private voice registry outside this source package.');
