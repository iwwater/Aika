...
export class HarnessForwarding implements ForwardingPort {
  ...
  private async validatePrepare(input: ForwardPrepare): Promise<Pick<ForwardRequest,'text'|'target'|'project'|'executor'|'plan'>> {
    ...
    if(selected){
      ...
      if(selected.detailRef.entryFile){
        ...
        if(rel==='..'||rel.startsWith('../')||isAbsolute(rel)||!(await stat(path)).isFile())return invalid();
      }
  ...
  async openNative(id:string):Promise<void>{
    ...
    await promisify(execFile)('/usr/bin/open',[url.toString()]);
  }
        ...
        if (record.project) {
          try {
            ...
            if (ref.entryFile) {
              ...
              if (rel === '..' || rel.startsWith('../') || isAbsolute(rel) || !(await stat(path)).isFile()) throw Error();
            }
