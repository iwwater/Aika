...
async function fixture(t:TestContext, actual?: {planner:WorkPlanner;classifier:WorkIntentClassifier}){
 ...
 t.after(async()=>{await conversation.close();await desktop.close();await desktopForward.close();await projects.close();store.close();await rm(root,{recursive:true,force:true});});
...
test('B37 restart retains latest dispatch and isolates another WeChat channel',async t=>{
 ...
 t.after(()=>restarted.close());await restarted.receive({text:'查询',messageId:randomUUID()},undefined,signal);assert.match(f.replies.at(-1)!,/persistent latest/);assert.equal(receipts.get(latest.id).phase,'completed');
 ...
 const other=await new WeChatTextConversation({...f.options,key:'other-channel',receipts:otherReceipts,forwarding:otherForward,send:async text=>{otherReplies.push(text);return true;}}).start();clearInterval((other as any).work.timer);t.after(()=>other.close());
