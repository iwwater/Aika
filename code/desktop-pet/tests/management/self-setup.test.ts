...
test('first view creates no credential storage; save returns only reference and preserves old key versions',t=>{
 ...
 const file=store.entries()[0]!.file;assert.equal(readFileSync(file,'utf8'),key);assert.equal(statSync(file).mode&0o777,0o600);
...
test('credential store rejects project-local paths and unsafe directories without rewriting existing files',t=>{
...
test('setup locks recover a verified dead local PID and retain live or malformed owners',t=>{
 ...
 const release=acquireSetupLock(file,scope);assert.equal(JSON.parse(readFileSync(file,'utf8')).pid,process.pid);
 assert.throws(()=>acquireSetupLock(file,scope),{code:'version_conflict'});release();
 writeFileSync(file,'unidentifiable',{mode:0o600});assert.throws(()=>acquireSetupLock(file,scope),{code:'unavailable'});assert.equal(readFileSync(file,'utf8'),'unidentifiable');
});
