...
test('second business rejection rolls back earlier writes, FTS, operation log, source suppression and outcome',async t=>{
  ...
  const db=new Database(f.filename);t.after(()=>db.close());assert.equal((db.prepare('SELECT count(*) n FROM memory_turn_outcomes').get() as {n:number}).n,0);
...
test('native error during a later change rolls back the complete plan and remains an error',async t=>{
  ...
  const db=new Database(f.filename);t.after(()=>db.close());db.exec("CREATE TRIGGER fail_plan BEFORE INSERT ON memory_records WHEN new.id='books' BEGIN SELECT RAISE(ABORT,'plan_write_failure'); END");
...
test('source versions are rechecked even if an external write does not advance maintenance epoch',async t=>{
  ...
  const db=new Database(f.filename);t.after(()=>db.close());db.prepare("UPDATE memory_records SET version=version+1,text='已经修正' WHERE character_id='companion' AND id='raw'").run();
