...
export function fixture(maxBytes = CONFIRMED_RETENTION.transcriptMaxBytes, artifactPackage = 'companion-step1-01') {
  ...
  return { directory, filename, options, open, setTime:(value:string)=>{time=value;}, cleanup:()=>{for(const store of stores) store.close(); rmSync(directory,{recursive:true,force:true});} };
}
