...
const openEmotion = (filename: string, store: SqliteMemoryStore) => {
...
test('additive component is idempotent, does not backfill, and survives restart', t => {
  ...
  let database = new Database(f.filename);
  ...
  let opened = { db: database, emotions: new SqliteEmotionState(database, store) }; t.after(() => opened.db.open && opened.db.close());
  ...
  store = f.open(); opened = openEmotion(f.filename, store);
...
test('nullable modality intensity, immutable snapshots, and independent U/M CAS are enforced', t => {
  ...
  const { db, emotions } = openEmotion(f.filename, store); t.after(() => db.close());
