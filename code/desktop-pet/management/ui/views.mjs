...
function settingsActions(a){return el('div',{class:'actions'},button(a.s.pending.has('settings')?'正在保存…':'保存配置，重启后生效',a.saveSettings,{id:'settings-save',class:'primary',disabled:!canSave(a)||a.s.settingsConflict||a.s.pending.has('settings')}));}
...
export function memoryView(a) {
  const {s}=a;
  const tabs=[['dynamics','记忆总览'],['fragments','来源与片段'],['traces','实际召回'],['policy','策略微调'],['maintenance','维护与遗忘'],['records','纠正记录'],['prompt','角色设定 Prompt'],['context','上下文试算']];
  return el('div',{},
    el('div',{class:'tab-actions md-tabs'},tabs.map(([key,label])=>button(label,()=>{s.section=key;a.render();const load=({records:a.loadRecords,prompt:a.loadPrompt,context:a.loadContext})[key];if(load)load();else a.memoryDynamics.load(key);},{id:'memory-'+key,'aria-pressed':s.section===key}))),
    s.section==='records'?recordsView(a):s.section==='prompt'?promptView(a):s.section==='context'?contextView(a):a.memoryDynamics.view(s.section));
}
...
export function modelsView(a) {
  ...
  return el('div',{},a.wake?.view(),versionStrip(settings),summaries,
...
function providerForm(a,slot,label){const {s}=a,current=s.snapshot.settings.effective.providers[slot],draft=s.settingsDraft.providers[slot],available=s.snapshot.adapters.filter(x=>x.slots.includes(slot)),adapter=available.find(x=>x.id===draft.adapterId),busy=s.pending.has('settings');
 ...
 const panel=card(label,el('div',{class:'config-summary'},el('small',{},'当前实际生效'),el('p',{},`${current.provider} / ${current.model}${current.voice?' · 音色 '+current.voice:''}`),el('small',{},'适配：'+current.adapterId)),el('p',{class:'subtle'},'下面是准备保存的配置；当前运行实例不会随表单变化。'));
